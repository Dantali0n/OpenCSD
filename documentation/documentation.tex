...
The BPF program compiled can now be used by user written program together with
the NVMe API of the prototype as illustrated in the steps of the striped arrows.
Each of these steps illustrates the following:
\begin{enumerate} \item The user program submits the BPF program in ELF format.
\item uBPF is used to start executing the BPF program providing an
implementation of the BPF API. \item The BPF program uses several of these API
calls that use SPDK, subsequently, SPDK uses the UIO driver to communicate with
underlying ZNS SSD. \item The data returned from this ZNS SSD is processed by
the driver and SPDK. \item The BPF program selects data to be returned to the
user program. \item The user program requests the return data and receives it.
\end{enumerate}

The NVMe API highlighted in blue is an hypothetical extension to the NVMe
command set. In this basic prototype it only exposes two commands. One, to send
a BPF program in ELF format, another to retrieve the return data of the BPF
program after it has finished executing. The interface of these commands is
shown in figure \ref{fig:nvm-commands}.
...
Similarly the BPF API is the hypothetical API available to BPF programs being
programmed unto a CSD. uBPF provides an environment to run these BPF programs.
By default no functions will be available from within the BPF program but uBPF
allows to easily define custom functions. The prototype design defines four
functions that primarily allow to read from the ZNS SSD and store the data
somewhere. Providing the BPF program a region to store data is essential as uBPF
only offers a limited 128 byte stack space as well as performing thorough bounds
checking upon each memory access. For this prototype a default value of 512K of
memory is provided to the BPF program.
...
As mentioned, the main method of the prototype performs a few demo operations to
illustrate its functionality. This includes filling the first zone with random
value integers. Followed-by, submitting a BPF program to read the first LBA of
this zone and return the number of integers above $RAND\_MAX / 2$. If the ZNS
SSD from QEMU is used this will result in 1024 integers being read of which
approximately 512 will be above this predefined value. Given the parameters from
QEMU the ZNS SSD will consist of 128 zones with each zone consisting of 32 LBAs
(sectors) each 4096 bytes large.
...
\item nvm\_csd \item spdk\_init \end{enumerate*}.

The \textit{arguments} component processes any argument passed when launching
the prototype, the parsed arguments are stored in an object that can be passed
to subsequent components.

One of these components is \textit{spdk\_init} which attaches to the first zoned
namespace it can find and its corresponding controller. This will be the ZNS SSD
used by SPDK. The SPDK instance to interact with these objects are made
available by \textit{spdk\_init} to the subsequent components.

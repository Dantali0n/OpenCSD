...
\footnotetext[15]{A practical implementation refers to an actual physical device
utilizing actual busses such as PCIe and NVMe containing the necessary
firmware to perform our simulated functionality in practice.}
...
compared to interpretation \cite{lukken2021zcsd}. However, JiT is only supported
on X86 based architectures and also disables the memory access verifier. This
access verifier is otherwise able to terminate the user submitted kernel
upon accessing memory outside its allocated memory segment.

Both execution path ways utilize the same mechanism for (decoupled) hooking 
...
The steps shown to achieve offloading can effectively be performed in
...
Our design moved from the development of an accelarator API to an artificial
...
Lastly, the use of \textit{LD\_PRELOAD} and \textit{RTLD\_NEXT} is arguably

...
for type in write read
    ...
    do
        ...
        results=$(cd /mnt; fio --name=global --rw=${type} --size=64k --bs=64k --name=fiotest64k --output-format=json+)
    ...
    do
        mount /dev/nvme0n1 /mnt
        results=$(cd /mnt; fio --name=global --rw=${type} --size=256k --bs=256k --name=fiotest256k --output-format=json+)
    ...
    do
        ...
        do
            mount /dev/nvme0n1 /mnt
            results=$(cd /mnt; fio --name=global --rw=${type} --size=${size} --bs=524288 --name=fiotest${size} --output-format=json+)
